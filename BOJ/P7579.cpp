...
int n, m, h, count1;
int box[10000][100];
vector<vector<int>>step(10000, vector<int>(100, 1000000));
...
int bfs() {
    ...
    while(!q.empty()) {
        ...
        if(y/n != 0) {
            if(box[y-n][x] == 0 && step[y-n][x] == 1000000) {
                q.push({y-n, x});
                step[y-n][x] = step[y][x] + 1;
                answer = max(answer, step[y-n][x]);
                count1++;
            }
        }
        if(y/n != h-1) {
